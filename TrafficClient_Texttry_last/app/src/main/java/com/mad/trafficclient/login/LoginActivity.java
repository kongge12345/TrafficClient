...
public class LoginActivity extends Activity  {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		initView();
		checkBox= (CheckBox) findViewById(R.id.cb_remember_pwd);
		sp=getSharedPreferences("num",MODE_PRIVATE);
		Boolean flag=sp.getBoolean("check",false);
		checkBox.setChecked(flag);
		if (checkBox.isChecked()){
			etUserName.setText(sp.getString("name","user1"));
			etUserPwd.setText(sp.getString("word","123456"));
		}
		initLiserter();
	}
	...
	private void initLiserter() {
		...
		checkBox.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
			...
			public void onCheckedChanged(CompoundButton compoundButton, boolean b) {
				if (b){
					sp.edit().putString("name",etUserName.getText().toString()).commit();
					sp.edit().putString("word",etUserPwd.getText().toString()).commit();
					sp.edit().putBoolean("check",b).commit();
				}
			}
		});
